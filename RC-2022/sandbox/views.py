...
def set_limit_resource(language, time, memory):
    time_limit = time
    memory_limit = memory
...
def execute(name, test_input, language, time, memory):
    ...
    if language == "py":
        ...
        a = subprocess.run(
            s,
            shell=True,
            preexec_fn=set_limit_resource(language, time, memory),
            stdin=test_input,
            stdout=f1,
            stderr=f2,
            text=True,
        ...
        if a.returncode == 0:
            ...
            exe = r"./{0}c".format(fp)
            a = subprocess.Popen(
                exe, shell=True, preexec_fn = set_limit_resource(language, time, memory), stdin=test_input, stdout=f1, stderr=f2, text=True
            )
            a.wait()
